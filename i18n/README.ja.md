...
**QuickClipboard** は、Tauri 2 + Rust + React で構築されたクロスプラットフォームのクリップボード強化ツールです（現在 Windows と Android に対応）。コピーした瞬間から自動的にテキスト、画像、リッチテキスト、ファイルを記録し、過去にコピーした内容をいつでも取り戻せます。記録だけでなく、OCR、WebDAV 同期、LAN 同期/転送などの機能も統合しており、日常業務の生産性を大幅に向上させます。
