...
社区版命令通过 `scripts/community-check.js` 临时移除私有插件依赖后运行 cargo，完成后自动恢复。截图插件（`src-tauri/plugins/screenshot-suite/`，私有子模块）内含 Rust 测试和前端测试（Vitest + Playwright），需拉取子模块后可用。

验证方式：
- **Rust 编译检查**: `npm run check:community`（社区版，无需私有插件）
- **Rust lint**: `npm run clippy:community`（社区版，无需私有插件）
- **Rust 单元测试**: `npm run test:rust`（社区版，无需私有插件）
